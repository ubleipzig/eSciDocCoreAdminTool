...
public class UserAddView extends CustomComponent implements ClickListener {
    ...
    private void addPasswordFields() {
        ...
        panel.addComponent(createLayout(ViewConstants.PASSWORD_CAPTION, passwordView.getPasswordField(), LEFT_MARGIN,
            true));

        panel.addComponent(createLayout(ViewConstants.RETYPE_PASSWORD_CAPTION, passwordView.getRetypePasswordField(),
            LEFT_MARGIN, true));
    }

    private Component createLayout(
        final String nameLabel, final AbstractTextField textField, final int leftMargin, final boolean required) {
    ...
    private void addOrgUnitWidget() {
        ...
        orgUnitWidget.withAddButton().addListener(new ShowOrgUnitsSelectionWidget(this));
        orgUnitWidget.withRemoveButton().withListener(new RemoveSeletecteOrgUnitListener(this));
    ...
    private Component createLayout(final VerticalLayout build) {
        ...
        final Label textLabel =
            new Label(Constants.P_ALIGN_RIGHT + ViewConstants.ORGANIZATION_UNITS_LABEL + Constants.P,
                Label.CONTENT_XHTML);
    ...
    private void addNameField() {
        panel.addComponent(LayoutHelper
            .create(ViewConstants.NAME_LABEL, nameField = new TextField(), LEFT_MARGIN, true));
    ...
    private void trySaveAndUpdateView() {
        try {
            ...
            Preconditions.checkNotNull(item, "Add new user to the list failed: %s", item);
            resetFields();
    ...
    private void showMessage() {
        app.getMainWindow().showNotification(
            new Notification("Info", "User Account is created", Notification.TYPE_TRAY_NOTIFICATION));
    }
    ...
    private void initSetOrgUnitsCommand(final UserAccount createdUserAccount) {
        ...
        setOrgUnitsCommand.setSeletectedOrgUnit(orgUnitWidget.getOrgUnitsFromTable());
    }

    private void setOrgUnitsFor(final UserAccount createdUserAccount) throws EscidocClientException {
        Preconditions.checkNotNull(createdUserAccount, "createdUserAccount is null: %s", createdUserAccount);
    ...
    private void showErrorMessageForEmptyPassword() {
        passwordView.getPasswordField().setComponentError(new UserError("Password can not be empty."));
    }
    ...
    private boolean isNameFieldValid() {
        return EmptyFieldValidator.isValid(nameField, "Please enter a " + ViewConstants.NAME_ID);
    }

    private boolean isLoginNameFieldValid() {
        return (EmptyFieldValidator.isValid(loginNameField, "Please enter a " + ViewConstants.LOGIN_NAME_ID));
    }

    private UpdatePasswordCommand updatePasswordCommand;

    private void setPasswordFor(final UserAccount createdUserAccount) throws EscidocClientException {
    ...
    private void initUpdatePasswordCommand(final UserAccount createdUserAccount) {
        ...
        updatePasswordCommand.setLastModificationDate(createdUserAccount.getLastModificationDate());
    }
    ...
    private UserAccount createUserAccount() throws EscidocException, InternalClientException, TransportException {
        final UserAccount createdUserAccount =
            userService.create(nameProperty.getValue(), loginNameProperty.getValue());
