...
import de.escidoc.admintool.view.user.password.UpdatePasswordView;
import de.escidoc.admintool.view.user.password.UpdatePasswordViewImpl;
...
public class UserEditForm extends CustomComponent implements ClickListener {
  ...
  public final void init() {
    ...
    panel.addComponent(LayoutHelper.create(ViewConstants.NAME_LABEL, nameField,
        LABEL_WIDTH, true));
    ...
    panel.addComponent(LayoutHelper.create("Active status", state, LABEL_WIDTH,
        false));
  ...
  private void addPasswordView() {
    panel.addComponent(updatePasswordView);
  }

  private void createUpdatePasswordView() {
    updatePasswordView = UpdatePasswordViewImpl.createView();
    updatePasswordView.addPasswordField();
    updatePasswordView.addRetypePasswordField();
    updatePasswordView.addMinCharValidator();

    final UpdatePasswordOkListener updatePasswordOkListener = new UpdatePasswordOkListener(
        updatePasswordView.getPasswordField(),
        updatePasswordView.getRetypePasswordField());
    ...
    updatePasswordView.addOkButton(updatePasswordOkListener);
    final ClickListener cancelListener = new Button.ClickListener() {
      ...
      public void buttonClick(ClickEvent event) {
        updatePasswordView.resetFields();
        updatePasswordView.removeErrorMessages();
      }
    };
    updatePasswordView.addCancelButton(cancelListener);
  }
  ...
  private void createAndAddRoleComponent() {
    ...
    final VerticalLayout rolesComponent = createLayout(
        ViewConstants.ROLES_LABEL, roleTable, 120, ROLE_LIST_HEIGHT, false,
        new Button[]{addRoleButton, removeRoleButton});
    panel.addComponent(rolesComponent);
  }

  private VerticalLayout createLayout(final String rolesLabel,
      final Table table, final int labelWidth, final int roleListHeight,
      final boolean b, final Button[] buttons) {

    final HorizontalLayout hLayout = new HorizontalLayout();
    hLayout.setHeight(roleListHeight + Constants.PX);
    hLayout.addComponent(new Label(" "));

    final Label textLabel = new Label(Constants.P_ALIGN_RIGHT + rolesLabel
  ...
  private final class AddRoleButtonListener implements Button.ClickListener {
    ...
    public void buttonClick(final ClickEvent event) {
      try {
        ...
        LOG.error(
            ViewConstants.AN_UNEXPECTED_ERROR_OCCURED_SEE_LOG_FOR_DETAILS, e);
      }
    }
  }

  private final class RemoveRoleButtonListener implements Button.ClickListener {
  ...
  private Collection<Grant> retrieveGrantsFor(final String userObjectId) {
    try {
      ...
      ModalDialog.show(app.getMainWindow(), e);
      LOG.error(ViewConstants.AN_UNEXPECTED_ERROR_OCCURED_SEE_LOG_FOR_DETAILS,
          e);
  ...
  private boolean isValid() {
    ...
    isValid = EmptyFieldValidator.isValid(nameField, "Please enter a "
        + ViewConstants.NAME_ID);
    isValid &= EmptyFieldValidator.isValid(loginNameField, "Please enter a "
  ...
  private void bindData() {
    ...
    modifiedBy.setPropertyDataSource(item.getItemProperty(PropertyId.MODIFIED_BY));
    ...
    createdBy.setPropertyDataSource(item.getItemProperty(PropertyId.CREATED_BY));
    bindRolesWithView();
  ...
  private void bindRolesWithView() {
    ...
    if (userGrants.isEmpty()) {
      ...
      grantContainer = new POJOContainer<Grant>(Grant.class,
          PropertyId.XLINK_TITLE, PropertyId.OBJECT_ID,
  ...
  public void changeState() throws InternalClientException, TransportException,
