...
qx.Class.define("org.escidoc.admintool.Application",
	...
	doc.add(scroller, {edge : -1});

	// container is layout root
	var theTree = this.getTree();
	this.container.add(theTree, {flex:1});
	this.container.add(theTree.getRoot().tabView, {flex:5});
	...
	getTree : function()
	{
		...
		resourcePageInfo.add(resourcePageInfo.html, {flex:1});
	...
	showFolderView : function(folder, tab)
	{
		...
		if(folder.tabView){
			...
			this.container.add(folder.tabView, {flex:5});
			if(tab){
			...
			//this.container.removeAt(1);
			//this.container.add(this.getDefaultTabView(), {flex:5});
			//this.container.getChildren()[1].getChildren()[0].html.setHtml("<h1>" + label + "</h1>");
		}
	}
	}
		});